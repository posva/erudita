...
export class DocumentationManager {
  ...
  private async saveToCache(packageName: string, version: string, info: DocumentationInfo, content: Record<string, string>) {
    const packageKey = this.getPackageKey(packageName, version);
  ...
  async listDocumentation(): Promise<Array<{
    name: string;
    version: string;
    lastUpdated: string;
    paths: string[];
  ...
  async getDocumentation(
    ...
    if (!cacheEntry) {
      throw new Error(`Documentation for ${packageKey} not found. Please update documentation first.`);
    }
